...
contract Core {
    ...
    function addStartupsToList(
        string memory _name,
        string memory _descrip,
        string memory _tagline,
        uint256 _amt
    ) external onlyStartupORInvestors {
    ...
    function investAmount(address _startupOwner) public payable onlyInvestors {
        ...
        require(
            ...
            "Startup needs no money"
        );

        uint256 valToEth = msg.value;

        uint256 calc = startupList[atIndex[_startupOwner]].amount;
    ...
    function changeIntrestRate(uint256 _amt) external onlyOwner {
        require(
            _amt > 0 && _amt < 20,
            "Intrest cant be Zero % and more than 20%"
        );
        interestRate = _amt;
    }
    ...
    function editName(string memory _name) external onlyStartupOwners {
        require(
            startupList[atIndex[msg.sender]].isActive == true,
            "Cant edit Inactive Startups"
        );
    ...
    function getOwnerStartups()
        external
        view
        onlyStartupOwners
        returns (S_Details memory)
    {
        return startupList[atIndex[msg.sender]];
    }

    function getOwnerStartupsAtIndex()
        external
        view
        onlyStartupOwners
        returns (uint256)
    ...
    function getInvestedStartupsIdx()
        external
        view
        onlyValidInvestor
    ...
    function getCurrentInvestemntNeeded()
        external
        view
        onlyStartupOwners
        returns (uint256)
    ...
    modifier onlyStartupOwners() {
        if (
            !(sbtFactory.isStartup(msg.sender) &&
                isAlreadyStartupOwner[msg.sender] == true)
        ) {
    ...
    modifier onlyValidInvestor() {
        if (
            !(sbtFactory.isInvestor(msg.sender) &&
                isValidInvestor[msg.sender] == true)
        ) {
